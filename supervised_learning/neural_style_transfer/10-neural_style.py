#!/usr/bin/env python3
"""Neural Style Transfer - Task 10: Variational Cost"""
import numpy as np
...
class NST:
    ...
    def variational_cost(generated_image):
        ...
        Args:
            generated_image: tf.Tensor shape (1, nh, nw, 3)

        Returns:
            the variational cost (total variation)
        """
        # Sum of absolute pixel differences in both spatial directions
        vert = tf.reduce_sum(
            tf.abs(generated_image[:, 1:, :, :] -
                   generated_image[:, :-1, :, :]))
        horiz = tf.reduce_sum(
            tf.abs(generated_image[:, :, 1:, :] -
                   generated_image[:, :, :-1, :]))
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01,
        ...
        """Generates the neural style transferred image with variational cost.

        Returns:
            (generated_image, cost) - best image numpy array and best cost
        """
